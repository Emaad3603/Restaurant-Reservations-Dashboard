...
@section('title', 'Create Reservation - Restaurant Reservations')
        ...
        <div class="card-body">
            <form action="{{ route('admin.reservations.store') }}" method="POST">
                ...
                <div class="row mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('guest_id') is-invalid @enderror" id="guest_id" name="guest_reservations_id" required>
                                ...
                                @foreach($guests as $guest)
                                    <option value="{{ $guest->guest_details_id }}" {{ old('guest_reservations_id') == $guest->guest_details_id ? 'selected' : '' }}>
                                        {{ $guest->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('people_count') is-invalid @enderror" id="people_count" name="people_count" value="{{ old('people_count') }}" min="1" required>
                            @error('people_count')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('hotel_id') is-invalid @enderror" id="hotel_id" name="guest_hotel_id">
                                ...
                                @foreach($hotels as $hotel)
                                    <option value="{{ $hotel->hotel_id }}" {{ old('guest_hotel_id') == $hotel->hotel_id ? 'selected' : '' }}>
                                        {{ $hotel->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('restaurant_id') is-invalid @enderror" id="restaurant_id" name="restaurant_id" required>
                                ...
                                @foreach($restaurants as $restaurant)
                                    <option value="{{ $restaurant->restaurants_id }}" {{ old('restaurant_id') == $restaurant->restaurants_id ? 'selected' : '' }}>
                                        {{ $restaurant->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('meal_type_id') is-invalid @enderror" id="meal_type_id" name="meal_types_id" required>
                                ...
                                @foreach($mealTypes as $mealType)
                                    <option value="{{ $mealType->meal_types_id }}" {{ old('meal_types_id') == $mealType->meal_types_id ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('special_request') is-invalid @enderror" id="special_request" name="special_request" rows="3">{{ old('special_request') }}</textarea>
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    <button type="submit" class="btn btn-primary">
                        <i class="bi bi-save me-1"></i> Create Reservation
                    </button>
                </div>
            </form>
        </div>
